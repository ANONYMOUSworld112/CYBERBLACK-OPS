        ...
        with Live(refresh_per_second=1, screen=True) as live:
            while True:
                try:
                    ...
                    # B-07: Wrap net_connections in try/except for AccessDenied
                    try:
                        conns = psutil.net_connections(kind="inet")
                    except (psutil.AccessDenied, psutil.Error):
                        conns = []
                        ct = Table(title="[bold cyan]Active Connections[/bold cyan]", box=box.SIMPLE, padding=(0, 1))
                        ct.add_column("Info", style="yellow")
                        ct.add_row("[yellow]Permission denied — run as admin/root to see connections[/yellow]")
                        renderables.append(ct)
                        conns = None  # signal to skip connection tables
                    ...
                    # B-07: last-resort catch so a single bad reading never kills the live view
                    pass
