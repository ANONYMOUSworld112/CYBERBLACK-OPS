...
def _sanitize_bundle_item_name(raw_name: str, seen_names: set[str]) -> str:
    """
    Sanitize a bundle item name to prevent path-traversal attacks.

    - Reduces to basename only (strips all directory components)
    - Rejects empty, '.', '..' names → defaults to 'unnamed.bin'
    - Appends numeric suffix on collision
    """
    # Take only the final path component (basename)
    safe = Path(raw_name).name if raw_name else ""

    # Reject empty, '.', '..' and any hidden traversal remnants
...
def unpack_bundle(data: bytes) -> list[PayloadObject]:
    ...
    All item names are sanitized to basenames to prevent path-traversal
    attacks (B-08 security fix).
    ...
    for item in items:
        raw_name = item.get("name", "unnamed.bin")
        mime = item.get("mime", "application/octet-stream")
        size = item.get("size", 0)
        expected_sha = item.get("sha256", "")
        meta = item.get("metadata", {})

        # B-08: sanitize name to prevent path traversal
        safe_name = _sanitize_bundle_item_name(raw_name, seen_names)

        if offset + size > len(data):
            raise StegoForgeError(f"Truncated bundle payload for item '{safe_name}'")
        ...
        if expected_sha and actual_sha != expected_sha:
            raise StegoForgeError(
                f"Integrity failure for bundle item '{safe_name}': expected {expected_sha[:12]}, got {actual_sha[:12]}"
            )
