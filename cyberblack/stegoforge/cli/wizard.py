...
from stegoforge.core.bundle import PayloadObject, create_payload_from_file
    ...
    try:
        ...
        if isinstance(payload, list):
            ...
            if dest_dir_str:
                ...
                for item in payload:
                    dest = (dest_dir / item.name).resolve()
                    # B-08 defense-in-depth: verify destination stays inside output dir
                    if not dest.is_relative_to(dest_dir):
                        console.print(f"[bold red]Path traversal blocked:[/bold red] item '{item.name}' would escape output directory. Skipping.")
                        continue
