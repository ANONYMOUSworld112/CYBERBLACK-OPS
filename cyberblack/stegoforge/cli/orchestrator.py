...
_ = (stegoforge.ciphers, stegoforge.methods)
...
def embed_operation(
    ...
    try:
        ...
        if payload_count > 1:
            msg_parts.append(f"Bundled {payload_count} files")
        if comp_used != "none":
        ...
        if is_bundle(raw_payload):
            ...
            if output_dir:
                ...
                for item in unpacked_bundle:
                    dest = (out_d / item.name).resolve()
                    # B-08 defense-in-depth: verify destination stays inside output dir
                    if not dest.is_relative_to(out_d):
                        raise StegoForgeError(
                            f"Path traversal blocked: item '{item.name}' would write outside output directory"
                        )
